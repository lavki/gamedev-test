...
<head>
  ...
  <link href="<?= base_url('images/favicon.png'); ?>" rel="shortcut icon" type="image/vnd.microsoft.icon">
        ...
        <div class="modal-body">
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              ...
              <ul>
                <li class="card-text" v-for="comment in post.coments">
                  {{ comment.user.personaname }} - <small class="text-muted">{{ comment.text }}</small> likes: ({{comment.likes}})
                  <comment-item v-for="comment in comment.comments" v-bind:comment="comment" v-bind:key="comment.id" v-bind:post_id="post.id"/>
                </li>
              </ul>
